...
make_narration.py 의 Qwen3-TTS 판. 선생님 본인 목소리로 생성한다.
출력 계약은 동일: assets/narration.mp3 + assets/narration/timing.json

edge-tts 판과 다른 점:
  - GPU 한 장이라 순차 생성 (edge-tts는 8개 병렬)
  - 약 3.4배 실시간 — 10분 나레이션이면 30분 이상 걸린다
  - 이미 만든 세그먼트는 건너뛴다(재개 가능). 처음부터 다시 만들려면 --force
"""
import json, subprocess, os, sys, time, argparse

QWEN_DIR = r"C:\ai\qwen3tts"
...
ap.add_argument("--force", action="store_true", help="이미 만든 세그먼트도 다시 생성")
ap.add_argument("--batch", type=int, default=4, help="한 번에 생성할 세그먼트 수 (VRAM 여유에 맞춰 조절)")
args = ap.parse_args()

import tts  # C:\ai\qwen3tts\tts.py — 모델/레퍼런스/생성 파라미터 기본값을 갖고 있다

jobs = [(sid, k, text) for sid, segs, _ in SCENES for k, text in enumerate(segs)]
todo = [j for j in jobs if args.force
        or not os.path.exists(os.path.join(OUT, f"{j[0]}_{j[1]}.wav"))]

print(f"[tts] {tts.MODEL_ID} / {tts.DTYPE} / {tts.GEN_DEFAULTS}")
print(f"[tts] ref={os.path.basename(tts.REF_AUDIO)}  세그먼트 {len(jobs)}개 중 {len(todo)}개 생성")
...
if todo:
    ...
    for i in range(0, len(todo), args.batch):
        ...
        paths = [os.path.join(OUT, f"{c[0]}_{c[1]}.wav") for c in chunk]
        t = time.time()
        try:
            tts.speak_batch(texts, paths)
        except Exception as e:                    # OOM 등 — 하나씩으로 자동 폴백
            print(f"  ! batch {len(chunk)}개 실패({type(e).__name__}) → 개별 생성", flush=True)
            for txt, p in zip(texts, paths):
                tts.speak(txt, p)
        done_n += len(chunk)
        el = time.time() - t0
        eta = el / done_n * (len(todo) - done_n)
        print(f"  [{done_n}/{len(todo)}] {time.time()-t:.0f}s  (남은 예상 {eta/60:.1f}분)", flush=True)
...
for sid, segs, extra in SCENES:
    ...
    for k, text in enumerate(segs):
        p = os.path.join(OUT, f"{sid}_{k}.wav")
        d = dur(p)
